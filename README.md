# Matemágica

Um jogo de resolver questões matemáticas para ajudar a praticar.

Tipos de questões à serem implementados:

- [X] soma simples
- [X] subtração simples
- [X] multiplicação simples
- [X] divisão inteira
- [X] soma com múltiplos dígitos
- [X] subtração com múltiplos dígitos
- [X] multiplicação com múltiplos dígitos
- [X] divisão com múltiplos dígitos
- [X] divisão fracionada
- [X] expressões
- [X] equações simples
- [X] sistemas de equações com duas variáveis
- [X] simplificar frações
- [X] expressões com frações
- [X] potenciação
- [X] raiz quadrada
- [X] expressões com raiz e exponenciação
- [ ] equação quadrática Simples
- [ ] equação quadrática Complexa
- [ ] Razão, proporção e porcentagem
- [ ] produtos notáveis
- [ ] Fatoração simples
- [ ] Identidades algébricas
- [ ] função
- [ ] limite
- [ ] questão de derivada simples
- [ ] questão de máximo e mínimo simples


Melhorias na UI:

- [X] selecionar tipo de questão
- [ ] centralizar opções
- [ ] embelezar butões e elementos
- [ ] adicionar contador de questões corretas
- [ ] adicionar animação que reage às respostas do usuário
